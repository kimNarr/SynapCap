...
# remote assets. OpenAI: v15.21.0, Google Gemini: v16.21.0 (CC0-1.0).
# Colours live in theme.py.
#
# Claude uses a hand-drawn radial burst as a stand-in for the official Claude
# mark — swap ``markup`` for the real path/geometry once it is vendored (see
# assets/PROVIDER_ICONS_NOTICE.md).
def _claude_burst_markup(rays: int = 11) -> str:
    lines = []
    for i in range(rays):
        angle = (i / rays) * 2 * math.pi - math.pi / 2
        x1 = 12 + 3.6 * math.cos(angle)
        y1 = 12 + 3.6 * math.sin(angle)
        x2 = 12 + 10.8 * math.cos(angle)
        y2 = 12 + 10.8 * math.sin(angle)
        lines.append(
            f'<line x1="{x1:.2f}" y1="{y1:.2f}" x2="{x2:.2f}" y2="{y2:.2f}"/>'
        )
    return (
        '<g stroke="{fg}" stroke-width="2.6" stroke-linecap="round" fill="none">'
        + "".join(lines)
        + "</g>"
        + '<circle cx="12" cy="12" r="3.6" fill="{fg}"/>'
    )
