...
6. **일관되게 고정한다** — 펼침 보기는 376px 고정 폭·가로 한 모델 집중 링으로 제공해 모델별 화면 차이를 없앤다.
...
- 고정 스케일 (`FIXED_WIDGET_PRESET`, `ui/widget.py`) — 펼침 폭 376px, 기준 수치 11px.
...
- 위젯 폭: 펼침 보기 376px 고정. 컴팩트 바만 표시 내용에 맞춰 폭을 조절한다.
