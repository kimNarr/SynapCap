...
**확정 — 컨셉 A "게이지 S"**: **닫힌 원형 게이지 트랙** 안에 스트로크 `S`, 그 위에 파란 채움
아크 ~66% + 선단 노드. 채움이 용량, 트랙(딤)이 전체. 작은 크기·다크에서 로딩 스피너(`#89B4FA`
245° 호)로 오인되던 문제 때문에 트랙을 270° 호 → 완전한 원으로 닫았다.
...
- **첫 실행·업데이트 직후**(`main._should_start_centered` — 저장된 위치가 전혀 없거나
  `last_seen_version`이 바뀜)에는 `widget.center_on_screen()`로 화면 중앙에 두고 그 좌표를 저장한다.
  이후 실행은 저장된 위치·모드를 그대로 복원한다.
- **창 제어는 두 모드에서 같은 순서·같은 아이콘**: `[트레이로]` `[모드 토글]` `[× 종료]`.
  - `[트레이로]` — 아래 화살표 + 트레이 칩(`create_to_tray_icon`), 양쪽 모드 동일 → `none`.
  - `[모드 토글]` — 펼침에선 가운데 정렬 가로 막대(`create_minimize_icon`) → 막대,
    막대에선 둥근 사각(`create_expand_square_icon`) → 펼침. 펼침 방향(위/아래)은 툴팁에.
  - 펼침 헤더는 그 앞에 `새로고침`·`설정` + 세로 구분선.
  - 트레이 아이콘 클릭·메뉴, 설정 "표시 모드" 콤보로도 전환.
- 트레이 아이콘 수치는 `settings["tray_metric"]`로 고른다. `highest`(기본)면 조회 가능한 모든
  한도 중 가장 높은 사용률, 특정 프로바이더 id면 그 프로바이더의 최고 사용률(5h/7d 중 큰 값)을
  정수로 표시하고 링과 같은 4단계 상태색을 쓴다. `none`이면 숫자 없이 앱 아이콘만 둔다.
  고른 프로바이더가 오류·미설치이거나 조회 전이어도 앱 아이콘으로 돌아가되 툴팁의 전체 내역은
  항상 유지한다. 설정 다이얼로그 "트레이 표시" 콤보(`최고 사용률` · 프로바이더별 · `숫자 표시 안 함`).
- 칩은 OS가 주는 아이콘 상자를 거의 꽉 채운다(`inset` ≈ 3%, 모서리 반경 30%). 숫자는 자릿수에 따라
  글리프 비율을 키운다(1자리 0.66 · 2자리 0.6 · 3자리 0.46 × 상자 크기) — 붐비는 트레이에서도 읽히도록.
...
- 설정창 하단은 `취소` / `저장` 2개. 테마·표시 모드·트레이 표시는 고르는 즉시 라이브 미리보기,
  `저장`이 전체 반영 + 파일 저장 + 닫기, `취소`가 미리보기를 되돌리고 닫는다. (별도 `적용` 버튼 없음)
...
**적용 (막대 뷰 시절)**: `75–89`는 `%` 굵게(`700`), `>= 90`은 `▲ 92%` 프리픽스 + 굵게.
2차: 스케일 4단계 + `< 60` 무채색(`usage_calm`), `USAGE_WARN` 60→75, `USAGE_CRIT` 80→90.

**적용 (집중 링 뷰, 현재)**: `focusRingMetric` 타일 **테두리로 신호를 준다.**
`< 75` 기본 `1px focus_metric_edge` / `75–89` `2px` severity 색 / `>= 90` `3px` severity 색.
"테두리 유무"가 calm↔주의를, "테두리 두께"가 warn↔crit을 색에 의존하지 않고 구분한다.
`test_elevated_windows_get_a_non_colour_border_signal`로 고정.
남은 아이디어: 링 아크 끝 노드(≥90), `▲` 병행.
