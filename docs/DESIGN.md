...
스팬으로 색을 분리하고 `/`는 딤, 뒤 `%`는 마지막 창 색. 프로바이더 그룹 사이에는 `1px`
세로 구분선(`_make_compact_divider`, `compact_divider` 토큰 — 다크 `#3A4152` / 라이트 `#7F899E`)을
프로바이더 사이와 앱 로고 뒤에 배치한다. 프로바이더 구간은 구분선 양쪽 9px, 앱 로고는
구분선 안쪽 5px 이상의 여백을 둔다. 기본 수치는 `Medium(500)`, 경고는 `600`, 위험은 `700`으로
올려 색상 외에도 단계가 읽히게 한다.
...
- 한 행: `5h`/`7d` 기간 라벨(`ink_dim`, 테두리 없음 — 버튼처럼 안 보이게) · 그래프 · `_condensed_reset`(`#8087A0`, `font−2`, `objectName="resetCountdown"`) · `%`.
...
- `%`: `objectName="usageValue"`, 4단계 스케일 색 + 굵기 위계 + `▲`(≥90). 고정폭 `max(48, val_size × 4 + 10)`
- 전환: 환경설정 "그래프 모양" `GraphShapePicker`(4종 미리보기 타일, 누르면 즉시 미리보기). 헤더 토글은 없음. `config.py`가 4값 검증.
...
| ID | 영역 | 문제 | 제안 | 대상 |
| --- | --- | --- | --- | --- |
| I-10 | 헤더 아이콘 | 300px 폭에 모노 아이콘 5개 + 버전 배지. 뷰 전환(원형) 아이콘 의미 불명 | ✅ **적용** — 환경설정 "그래프 모양"을 4종 미리보기 타일(`GraphShapePicker`)로 바꾸고 헤더 `view_btn` 제거. 헤더는 앱 동작(새로고침·설정) → `1px` 세로 구분선(`headerDivider`, `line`) → 창 제어(최소화·닫기) 4개 + 버전 배지. 컴팩트 바의 `⌄`·`✕`도 같은 구분선. `···` 오버플로는 불필요 |
...
2. **큰 재설계** (시안 `SynapCap UI 시안` 참고, 오너 확인 필요):
   ...
   - ~~헤더 간소화(I-10)~~ — ✅ 설정 "그래프 모양"을 4종 미리보기 타일로, 헤더 `view_btn` 제거(아이콘 4개)
