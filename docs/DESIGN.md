...
| ID | 영역 | 문제 | 제안 | 대상 |
| --- | --- | --- | --- | --- |
| I-10 | 헤더 아이콘 | 300px 폭에 모노 아이콘 5개 + 버전 배지. 뷰 전환(원형) 아이콘 의미 불명 | ✅ **적용** — 환경설정 "그래프 모양"을 4종 미리보기 타일(`GraphShapePicker`)로 바꾸고 헤더 `view_btn` 제거. 헤더는 새로고침·설정·최소화·닫기 4개 + 버전 배지. `···` 오버플로는 불필요 |
