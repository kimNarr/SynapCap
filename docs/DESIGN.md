...
6. **일관되게 고정한다** — 펼침 보기는 336px 고정 폭·가로 한 모델 집중 링으로 제공해 모델별 화면 차이를 없앤다.
...
- 고정 스케일 (`FIXED_WIDGET_PRESET`, `ui/widget.py`) — 펼침 폭 336px, 기준 수치 11px.
...
- 위젯 폭: 펼침 보기 336px 고정. 컴팩트 바만 표시 내용에 맞춰 폭을 조절한다.
- 엣지 스냅 거리 `EDGE_SNAP_DISTANCE = 48`.
- 위치 보정은 `QScreen.geometry()` 전체를 기준으로 수행한다. 따라서 작업 표시줄·Dock과 겹칠 수 있지만
  화면 밖으로는 나가지 않는다.
