...
| I-05 리셋 열 정렬 | ✅ `setFixedWidth` — 단 **I-17** 참고 |
| I-06 트랙 대비 | ✅ `UsageBar` 트랙 `#1C2130` / `#3A4152` |
| I-09 노랑 경고색 | 굵기 가중으로 부분 커버, 색 변경은 미정 |
...
**현상**: 프로바이더 이름 위아래 공간은 넉넉, 행 사이는 빡빡. 카드 상/하단 패딩 비대칭.

**대상**: `ui/widget.py` — `_build_provider_cards` / 카드 레이아웃 여백,
`_render_usage_rows` 행 `contentsMargins`.

**구현안**:
- 이름(`.top`) 아래 여백을 줄여 이름이 자기 행들에 붙게.
- 카드 자체의 상/하 패딩을 대칭으로.
- 카드 사이 구분선(`#20242D`) 위아래 여백을 키워 카드가 "한 덩어리"로 읽히게.
- 구체 수치는 시각 조정.

**수용 기준**: 각 프로바이더 카드가 하나의 그룹으로 명확히 묶여 보인다.

**테스트**: 시각 확인. `test_*` 에서 카드 높이 하한을 검사하는 것들 회귀 확인.

---

#### I-09 · 노랑 경고색 대비 약함

**현상**: `#F9E2AF` (60–79 경고)가 어두운 바탕에서 파랑·빨강보다 덜 띈다.
65% 주간을 놓치기 쉽다.

**대상**: `ui/widget.py` — `_usage_color`.

**구현안**: 경고색을 `#F9E2AF` → `#FAB387` (peach, 채도↑) 로. 또는 I-03의 굵기 가중으로 커버.
`_set_status_badge` 의 `waiting` 색과의 일관성도 함께 고려(같이 바꿀지).

**수용 기준**: 60–79 구간 막대·수치가 여유(파랑) 구간과 명확히 구분.

**테스트**: `_usage_color` 반환값 테스트가 있으면 갱신.

---

#### I-17 · 리셋 열 고정폭이 한국어 상태 문구를 자름 (I-05의 부작용)

**현상**: I-05로 `reset_label.setFixedWidth(~48px)`를 넣었는데, 이 폭은 `"5d 19h"` 같은
카운트다운엔 맞지만 `"초기화 확인 중"`, `"리셋 시각 미상"` 같은 상태 문구는 `"초기화 확…"`으로
잘려 읽을 수 없다. (한도 초기화 직후·조회 실패 시 발생)

**대상**: `ui/widget.py` — bar 분기 `reset_label` `setFixedWidth`.

**구현안** (택1):
- (a) 상태 문구를 짧게: `"초기화 확인 중"` → `"확인 중"`, `"리셋 시각 미상"` → `"미상"`.
  `_condensed_reset` 또는 표시 직전에 매핑. 툴팁엔 원문 유지.
- (b) 고정폭을 상태 문구까지 수용하도록 넓힘 (`~72px`) — 막대 시작선이 오른쪽으로 밀림.
- (a) 권장 (정렬 유지 + 카운트다운 폭 유지).

**수용 기준**: 모든 리셋/상태 문구가 잘리지 않고 표시. 정상 카운트다운의 막대 시작선은 그대로.

**테스트**: `tests/test_widget.py` — `test_missing_reset_is_shown_explicitly` 에서
`reset_label.text()` 가 잘리지 않은 값인지(폭 안에 fits) 확인, 또는 매핑 결과 문자열 검증.
...
2. **다음**: I-17(리셋 문구 잘림), I-08(카드 여백), I-09(노랑 경고색), 그리고 P3.
