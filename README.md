...
설정에서 시작 시 업데이트 확인을 끌 수 있습니다. 업데이트 파일은 사용자가 설치를 선택한 경우에만 다운로드합니다.
...
5. 접기 버튼은 위젯을 프로바이더별 사용률이 보이는 가로형 컴팩트 바로 바꿉니다. 두 한도를 표시하도록 설정했다면 `5시간/주간` 순서의 `15%/50%` 형식으로 표시되며, 폭은 콘텐츠에 맞춰 자동 조절됩니다. 펼치기 버튼으로 원래 크기로 돌아옵니다. `×` 버튼은 프로그램을 완전히 종료합니다. 트레이 메뉴에서도 표시·숨기기와 종료를 제어할 수 있습니다.
...
- 설정은 사용자 컴퓨터에만 저장됩니다.
- 업데이트 확인은 시작 시 GitHub의 최신 Release 정보만 조회하며 설정에서 끌 수 있습니다. 사용자가 설치를 선택하면 운영체제별 자산과 체크섬을 공식 Release에서 내려받아 검증합니다.
