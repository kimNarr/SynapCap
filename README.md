...
- 상단 탭에서 모델을 고르는 가로 한 모델 집중 듀얼 링 그래프
- 리셋까지 남은 시간을 간결하게 표시
- 항상 위에 고정 및 갱신 주기 설정
- 펼친 위젯은 384px 고정 폭의 가로 한 모델 집중 링 보기로 제공
- 프로바이더별 5시간·주간 한도 표시 여부 설정
- 선택형 사용량 임계치 알림: 기본 비활성화, 50~100% 기준 설정, 같은 한도는 하락 후 재상승할 때만 다시 알림
- 상태 배지를 눌러 감지된 CLI 경로·최근 조회·설치 및 로그인 안내를 확인하고 토큰 없는 진단 정보 복사
- 펼침·막대·트레이만 3단계 표시 모드와 모드별 마지막 위치 저장
- 시스템 트레이에서 가장 높은 사용률을 숫자·상태색으로 확인하고 표시 모드, 새로고침, 업데이트 확인, 재시작, 설정, 종료 제어
- 트레이와 환경설정에서 버그·기능 제안·기타 의견용 GitHub Issue 작성 화면 열기
- 작업표시줄·Alt+Tab 또는 Dock에는 표시하지 않는 트레이 전용 동작
- 상단 버전 배지와 트레이 메뉴에서 시작하는 검증형 원클릭 업데이트
- 어두운 바탕에서도 또렷한 브랜드 아이콘과 설정한 5시간·주간 사용률을 함께 표시하는 반응형 컴팩트 모드
- 화면 위치에 따라 위·아래 펼침 방향을 자동 선택하고 가장자리 가까이에서 달라붙는 위젯 스냅
- 작업 표시줄·Dock을 제외한 사용 가능 화면 경계를 넘지 않도록 자동 보정
- 설정을 저장하기 전에 크기·표시 옵션을 위젯에 바로 적용해 보는 적용 기능
- Windows·macOS 공통 환경설정 디자인과 종료 확인창
- Windows CLI 콘솔 숨김 및 시간 초과 프로세스 정리
- 중복 실행 시 새 프로세스를 종료하고 기존 위젯을 앞으로 표시
- 업데이트 후 최초 한 번 새 버전 안내와 정확한 GitHub 릴리스 링크 제공
...
새 버전이 있으면 상단 버전 배지와 트레이 메뉴에 표시됩니다. 업데이트를 누르면 현재 운영체제에 맞는 공식 GitHub Release 설치 파일과 `SHA256SUMS.txt`를 내려받아 SHA-256을 확인합니다. 파일명, Release 주소 또는 체크섬이 하나라도 다르면 설치하지 않고 기존 버전을 그대로 유지합니다.

- **Windows:** 관리자 권한 확인(UAC) 후 설치를 자동으로 진행하고 SynapCap을 다시 실행합니다.
- **macOS:** 현재 앱이 Apple 개발자 서명·공증 전이므로 검증된 DMG까지 자동으로 엽니다. DMG에서 SynapCap을 **응용 프로그램** 폴더로 드래그해 기존 앱을 교체합니다.

설정에서 자동 업데이트 확인을 끌 수 있습니다. 활성화하면 시작 4초 후와 실행 중 6시간마다 새 버전을 확인하며, 트레이의 **업데이트 확인**은 설정과 관계없이 언제든 사용할 수 있습니다. 업데이트 파일은 사용자가 설치를 선택한 경우에만 다운로드합니다.

> [!NOTE]
> 원클릭 업데이트 기능이 들어 있지 않은 `v0.1.4` 이하에서는 이번 한 번 최신 Release 설치 파일을 직접 받아야 합니다. 원클릭 업데이트가 포함된 버전으로 올라온 뒤부터는 앱 안에서 다음 업데이트를 진행할 수 있습니다.
...
4. 톱니바퀴 버튼에서 프로바이더, 표시할 한도, 갱신 주기, 표시 모드, 선택형 사용량 알림, 업데이트 확인 여부를 설정합니다. 펼친 위젯은 384px 고정 폭의 가로 한 모델 집중 링 보기입니다.
5. 접기 버튼은 위젯을 프로바이더별 사용률이 보이는 가로형 컴팩트 바로 바꿉니다. 두 한도를 표시하도록 설정했다면 `5시간·주간` 순서의 `15 / 50%` 형식으로 표시되며, 폭은 콘텐츠에 맞춰 자동 조절됩니다. 트레이 메뉴의 **표시 모드**에서 펼침·막대·트레이만을 고를 수 있고, 트레이 아이콘을 클릭하면 마지막으로 사용한 창 모드가 복원됩니다. `×` 버튼은 프로그램을 완전히 종료합니다.

각 카드와 환경설정 목록에는 Codex, Gemini, Claude의 식별용 브랜드 아이콘이 표시됩니다. 조회에 실패하면 카드에 `설치 필요`, `로그인 필요`, `시간 초과` 또는 `조회 오류`가 표시되며 마우스를 올려 상세 원인을 확인할 수 있습니다. macOS 설치본은 Finder에서 실행해도 Homebrew와 사용자 로컬 CLI 경로를 자동 탐색합니다. 사용량 링 위로 마우스를 올리면 사용량 상세 툴팁이 즉시 표시되며, 기간 표식과 리셋 시간은 읽는 흐름을 방해하지 않도록 툴팁을 표시하지 않습니다.
...
| `조회 오류` | 출력 형식 변경 등 그 밖의 오류 | 상태 배지를 눌러 감지 경로와 해결 안내를 확인합니다. 필요하면 토큰이 없는 진단 보고서를 복사해 GitHub Issue에 첨부합니다. |
...
- 설정은 사용자 컴퓨터에만 저장됩니다.
- 복사형 진단 보고서는 운영체제·앱 버전·프로바이더·CLI 경로와 조회 오류만 포함하며 토큰과 API 키를 수집하지 않습니다.
- 자동 업데이트 확인은 시작 4초 후와 실행 중 6시간마다 GitHub의 최신 Release 정보만 조회하며 설정에서 끌 수 있습니다. 트레이에서는 수동으로 즉시 확인할 수 있습니다. 사용자가 설치를 선택하면 운영체제별 자산과 체크섬을 공식 Release에서 내려받아 검증합니다.
...
Windows에서 일반 CLI 콘솔은 숨김 처리합니다. Claude는 공식 엄격 MCP 설정으로 실행하며 Antigravity는 macOS Keychain을 포함한 기존 사용자 인증을 그대로 사용합니다. SynapCap의 짧은 Antigravity 조회에서는 별도 자동 업데이터를 실행하지 않습니다. macOS에서는 CLI 탐색 중 Music 같은 보호 폴더나 네트워크·외장 볼륨도 조회하지 않습니다.
...
├─ diagnostics.py          # 토큰 없는 로컬 CLI 진단 보고서
├─ release_notes.py        # 업데이트 후 1회 변경 사항 안내
├─ single_instance.py      # 중복 실행 방지와 기존 위젯 활성화
├─ usage_alerts.py         # 사용량 임계치 알림 상태 관리
├─ ui/                     # 위젯, 설정, 트레이, 아이콘
...
- [버그 신고와 기능 제안](https://github.com/kimNarr/SynapCap/issues/new/choose)
