...
`Cx`, `G`, `Cl` 배지는 각각 Codex, Gemini, Claude를 의미합니다. 조회에 실패하면 카드에 `조회 오류`가 표시되며 마우스를 올려 CLI 미설치, 로그인 필요, 시간 초과 등의 원인을 확인할 수 있습니다. 사용량 막대와 리셋 정보의 툴팁은 마우스를 올리는 즉시 표시됩니다.
