...
- 리셋까지 남은 시간을 간결하게 표시
- 항상 위에 고정 및 갱신 주기 설정
- 펼친 위젯은 336px 고정 폭의 가로 한 모델 집중 링 보기로 제공
...
- 시스템 트레이에서 표시, 새로고침, 업데이트 확인, 재시작, 설정, 종료
...
- 전체 화면 경계를 넘지 않도록 자동 보정하며, 작업 표시줄·Dock 위에도 자유롭게 배치 가능
...
4. 톱니바퀴 버튼에서 프로바이더, 표시할 한도, 갱신 주기, 선택형 사용량 알림, 업데이트 확인 여부를 설정합니다. 펼친 위젯은 336px 고정 폭의 가로 한 모델 집중 링 보기입니다.
5. 접기 버튼은 위젯을 프로바이더별 사용률이 보이는 가로형 컴팩트 바로 바꿉니다. 두 한도를 표시하도록 설정했다면 `5시간·주간` 순서의 `15 / 50%` 형식으로 표시되며, 폭은 콘텐츠에 맞춰 자동 조절됩니다. 펼치기 버튼으로 원래 크기로 돌아옵니다. `×` 버튼은 프로그램을 완전히 종료합니다. 트레이 메뉴에서는 표시·숨기기, 업데이트 확인, 재시작과 종료를 제어할 수 있습니다.
